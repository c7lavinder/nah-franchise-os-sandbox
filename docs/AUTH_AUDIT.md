...
## /api/territory-owners/_, /api/zorakle/_, /api/knowledge/_, /api/scorecards/_, /api/research/_, /api/metrics/_, /api/suggestions/_, /api/sub-task-logs/_
...
### WEBHOOK_SHARED_SECRET — ACTIVATED (Tier 1 #7, 2026-04-28)

`WEBHOOK_SHARED_SECRET` added to Vercel (production, preview, development).
GHL webhooks configured with `x-webhook-secret` header:

- ContactCreate → `/api/webhooks/ghl/contacts` (canonical new-prospect handler)
- InboundMessage, OutboundMessage, OpportunityStageUpdate → `/api/webhooks/ghl`

Remaining providers (DocuSign, Trainual, Zorakle, Google Meet, form-submission, payment) still need provider-side config before their webhooks will pass verification.
