...
| Cron                                                             | Retires when                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| research-territories (Sun 3am UTC)                               | NO native equivalent exists (checked: "territory-market" appears nowhere in the MS Frandev module; the native Sun-2am Hangfire slot is CONTACT research). CORRECTION (s109): it does NOT write MS tables directly — it writes Supabase `territory_market_data` (lib/agents/territory-market.ts) and reaches MS only via the nightly push, so push-frandev must outlive this cron or its output stops landing natively. RULED (Corey, s109): research stays — MS actively reads it (FrandevService.Territories + PostCallPrompts). The JSON-truncation bug (11 errors/8 days) was fixed s109: max_tokens 4096→16000 + explicit truncation error. Retires when a native territory-research agent is built. |
...
1. Build the native tail: related-people panel, journey-doc upload,
   ...
   STATUS (s109): notes UI was ALREADY NATIVE (MS PRs #700+#702, all
   three record pages — the handoff was stale). Journey docs app-side
   use Supabase Storage, not S3 (the native build uses MS's S3 infra).
   RULINGS (Corey, s109): emails/team/messages → FREEZE, Gunner's
   surfaces are the native home (evidence: contact_emails stale since
   Jul 1, 9 SMS ever, no team table). Pipeline CONFIG → FREEZE, use
   Gunner settings if ever needed (untouched since Apr 9 creation).
   Webhook fates (incl. Zorakle receiver) → DEFERRED, Corey/Ben to
   think through ("we will build these in app i think"); zero events
   ever on all 10 candidate hooks, contact_zorakle_data empty.
