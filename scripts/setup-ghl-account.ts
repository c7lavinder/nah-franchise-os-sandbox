...
async function setupPipelines(): Promise<GHLPipeline[]> {
  ...
  const existingNames = new Set(existing.pipelines.map((p) => p.name));
  const createdPipelines: GHLPipeline[] = [];

  for (const pipeline of PIPELINES_TO_CREATE) {
    if (existingNames.has(pipeline.name)) {
      log("SKIPPED", "Pipeline", pipeline.name, "already exists");
      const found = existing.pipelines.find((p) => p.name === pipeline.name)!;
    ...
    try {
      ...
      if (msg.includes("401") || msg.includes("not authorized")) {
        log("FAILED", "Pipeline", pipeline.name, "PIT key cannot create pipelines — needs OAuth or manual creation in GHL UI");
      } else {
...
async function setupCustomFields() {
  ...
  for (const field of OPPORTUNITY_FIELDS) {
    ...
    try {
      ...
      log("FAILED", "Opportunity Field", field.name, err instanceof Error ? err.message : String(err));
    }
  }
}
...
async function setupTags() {
  ...
  // GHL doesn't have a clean "list all tags" endpoint, so we just try to create
