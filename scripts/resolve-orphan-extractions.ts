 ...
 * one scope pointer (contact_id, journey_id, or TerritorySlug).
 *
...
async function main(dryRun: boolean) {
  ...
  const { data: calls } = await supabase.from("calls").select("id, contact_id, TerritorySlug").in("id", callIds);
  const callMap = new Map((calls ?? []).map((c) => [c.id, c]));

  // 4. For rescuable calls (have contact_id or TerritorySlug), resolve journey.
  const rescuableContactIds = [...new Set((calls ?? []).map((c) => c.contact_id).filter(Boolean) as string[])];
  const { data: journeys } = await supabase
    .from("journeys")
    .select("id, primary_contact_id")
    .in("primary_contact_id", rescuableContactIds.length > 0 ? rescuableContactIds : ["__none__"]);
  ...
  for (const [callId, extractionIds] of byCall) {
    ...
    const rescuable = call.contact_id || call.TerritorySlug;
    if (rescuable) {
      const patch: { contact_id?: string; journey_id?: string; TerritorySlug?: string } = {};
      if (call.contact_id) patch.contact_id = call.contact_id;
      if (call.contact_id && contactToJourney.has(call.contact_id))
        patch.journey_id = contactToJourney.get(call.contact_id);
      if (call.TerritorySlug) patch.TerritorySlug = call.TerritorySlug;
      if (dryRun)
        console.log(
          `  WOULD RESCUE ${extractionIds.length} (call ${callId.slice(0, 8)}) patch=${JSON.stringify(patch)}`
        );
      else {
