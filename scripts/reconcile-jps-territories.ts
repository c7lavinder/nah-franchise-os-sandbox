 ...
 * owns multiple active territories, only one TerritorySlug made it
...
async function main(dryRun: boolean) {
  ...
  const owners = await fetchAll<{ ghl_contact_id: string | null; TerritorySlug: string }>(
    "territory_owners",
    "ghl_contact_id, TerritorySlug",
  ...
  const jps = await fetchAll<{
    ...
    "journey_id, pipeline_id, TerritorySlug, current_stage_id, current_sub_task_id, current_sub_task_started_at, entered_pipeline_at, entered_current_stage_at, assigned_user_id, is_active",
    // @ts-expect-error builder
    (q) =>
      q
        .in("journey_id", journeyIds)
        .eq("is_active", true)
        .in("pipeline_id", [RUNWAY_PIPELINE_ID, ONBOARDING_PIPELINE_ID])
  );
  ...
  for (const row of jps) {
    ...
    if (row.TerritorySlug) set.add(row.TerritorySlug);
  ...
  for (const [journeyId, ownedSlugs] of ownedByJourney) {
    for (const pipelineId of [RUNWAY_PIPELINE_ID, ONBOARDING_PIPELINE_ID]) {
      ...
      for (const slug of missing) {
        ...
        if (dryRun) {
          console.log(
            `  WOULD INSERT: journey=${journeyId.slice(0, 8)}.. pipeline=${pipelineId === RUNWAY_PIPELINE_ID ? "runway" : "onboarding"} territory=${slug}`
          );
        } else {
