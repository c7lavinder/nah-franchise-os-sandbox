...
## Post-Wrap Addendum 2 — unified pipeline page (Corey's next directive, same session)

**Branch `frandev-inventory-pipelines`** (pushed, stacked on frandev-dayhub-calls-crm; PR awaits Corey's word — updated review brief: https://claude.ai/code/artifact/abce677f-7ddf-4ac7-b3df-7a91ce89e238). The Gunner INVENTORY page now workspace-forks like the Day Hub: FranDev workspace = FranDev pipeline strips (GetStageBar live counts), journey rows (GetLeadRows: stage chip/source/urgency/days), stage-dot filtering (fd.{stageId} keys), search + urgency/name sorts + paging; title becomes "Pipeline · N journeys". Row click expands the SUB-STAGE CHECKLIST (`_FrandevLeadPanel` over GetQuickPanel) + upcoming + contacts. **AUTO-MOVE** (new `CompleteSubTaskAuto`, composed from existing journaled writes): sub-stage check → LogSubTask + pointer hops to next incomplete (BoardMove); last sub-stage → auto-advance stage (AdvanceJourneyStage, terminal refused). Gunner path byte-identical; bulk bar / Add-property / Data Issues / quick-filter stickies / prospect expander gated off in FranDev (int-vs-Guid JS hazard). Kanban toggle (frandev) → /frandev/pipeline. Verified live on 7128: both workspaces, stage filter (41 = Engagement), sub-stage check auto-moved to "Intro Call" + reverted. Not exercised: live stage auto-advance (composes the E2E-tested advance). Test residue: mark woodring's pointer sits on Intro Call w/ fresh timer. NOTE: original review-brief artifact URL rejected redeploys (org-mismatch error) — v2 brief published fresh.

## Post-Wrap Addendum 3 — pipeline strips → panel registry + Chad's FranDev access

**Corey's ruling confirmed and built (2026-07-22): pipelines compose per user like panel cards.** Second commit on `frandev-inventory-pipelines` (8f0a93147): strips are registry rows (PageKey='pipeline', Slot='strip', catalog `PipelineStrips`, migration -166) — gunner strips LIVE behind Gunner permission (fallback = same 4, byte-identical for current users), 5 FranDev strips beta corey@ behind Frandev permission. REVISED same day (Corey: 'I want the switch, not all at once'): the registry decides what a user is GRANTED; the workspace switch is a LENS deciding which KIND displays (gunner lens = property strips only, frandev lens = FranDev strips only; single-kind users see theirs, no switch; composing WITHIN a kind stays registry-driven — commit f44dc9e2f). Verified both lenses live; stage-dot clicks match dot counts on both sides. Corey also granted explicit Frandev permission on dev (was admin-shortcut only) — holders: chad@/corey@/matt@.
**Chad Arnold FranDev access GRANTED on dev** (Corey's ask): `chad@newagainhouses.com` (UserId 152) → Frandev permission = 1 (UserPermissionConfiguration, REPLACE — takes effect at his next login), and the **FranDev left-nav entry (MasterSuiteUI_NavigationMenuItems Id 76, /frandev) flipped Enabled=1** — it was disabled for EVERYONE; still permission-gated so only Frandev holders (matt@, chad@, admins) see it. Id 77 (/v2/frandev) left disabled. NOTE: grants are DEV-DB — production needs the same two flips at rollout. The consolidated Gunner-page FranDev workspaces (day hub / calls / pipeline strips) remain beta corey@-only — add chad@ to BetaUserIds CSVs when Corey wants him on those.
