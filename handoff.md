...
**Corey's ruling confirmed and built (2026-07-22): pipelines compose per user like panel cards.** Second commit on `frandev-inventory-pipelines` (8f0a93147): strips are registry rows (PageKey='pipeline', Slot='strip', catalog `PipelineStrips`, migration -166) — gunner strips LIVE behind Gunner permission (fallback = same 4, byte-identical for current users), 5 FranDev strips beta corey@ behind Frandev permission. Someone granted both sees BOTH sets in registry order; the ROW LIST follows the stage filter's namespace (fd.\* → journeys, else properties), workspace cookie breaks the tie only when unfiltered + both granted. Verified: 9 strips compose for corey@; acq.stage1 → 33 property rows; fd Engagement → 41 journey rows; strips persist across both.
