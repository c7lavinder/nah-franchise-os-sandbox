...
export async function runPostCallAgent(
  ...
  if (extractions && extractions.extractions.length > 0) {
    try {
      ...
      if (autoSaveResult.saved > 0) {
        console.log(`[post-call-agent] ${callId} auto-saved ${autoSaveResult.saved} high-confidence extractions`);
      }
    } catch (err) {
  ...
  // 3c. Mark contact brief as stale (will be regenerated by nightly cron)
