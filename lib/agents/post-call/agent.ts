...
import { processCommitments } from "./process-commitments";
...
export async function runPostCallAgent(
  ...
  // 3c. Process commitment extractions into commitments table
  if (extractions && extractions.extractions.length > 0) {
    try {
      const commitmentResult = await processCommitments(callId, context.contactId, null, supabase);
      if (commitmentResult.created > 0) {
        console.log(`[post-call-agent] ${callId} created ${commitmentResult.created} commitments`);
      }
    } catch (err) {
      errors.push(`commitments: ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  // 3d. Mark contact brief as stale (will be regenerated by nightly cron)
