 ...
 * still missing contact_id / TerritorySlug so we never clobber a prior
 * higher-signal match.
 */

import { createServerClient } from "@/lib/supabase/server";
import { resolveCallParticipants, createSupabaseResolverDb, type ParticipantSignal } from "../resolve-participants";
...
export async function reconcileCall(callId: string): Promise<void> {
  ...
  const result = await resolveCallParticipants({ participants: signals, meeting_title: null, source: "read_ai" }, db);
  ...
  if (result.contact_id || result.TerritorySlug || result.journey_pipeline_state_id) {
    ...
    if (call) {
      ...
      if (!call.TerritorySlug && result.TerritorySlug) callUpdates.TerritorySlug = result.TerritorySlug;
