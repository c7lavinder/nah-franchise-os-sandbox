 ...
 *
 * Per §1.13: Re-engaged → spawn new Sales Pipeline entry.
 * User picks fresh start (Stage 1) or resume at most recent prior stage.
 */
...
export async function POST(
  ...
  try {
    ...
    // Check if already has active Sales entry
    ...
    if (mode === "resume") {
      // Find most recent closed Sales entry
    ...
    // Get first sub-task of target stage
    ...
    // Create new Sales pipeline state
    ...
    await supabase.from("pipeline_stage_history").insert({
      contact_pipeline_state_id: newState.id,
      from_stage_id: null,
