...
export async function GET(request: NextRequest, { params }: { params: Promise<{ TerritorySlug: string }> }) {
  ...
  const { data, error } = await supabase.from("territory_market_data").select("*").eq("TerritorySlug", TerritorySlug);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ TerritorySlug: string }> }) {
  const { TerritorySlug } = await params;
  const supabase = createServerClient();
  ...
  const { error } = await supabase.from("territory_market_data").upsert(
    {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ TerritorySlug: string }> }) {
  ...
  const { error } = await supabase
    ...
    .upsert(rows, { onConflict: "TerritorySlug,field_name" });
