 ...
 *
 * Body: { TerritorySlug, ghl_contact_id, role?: "owner" | "co-owner" }
 */
...
export async function POST(request: NextRequest) {
  ...
  if (!body.TerritorySlug || !body.ghl_contact_id) {
    return NextResponse.json({ error: "TerritorySlug and ghl_contact_id required" }, { status: 400 });
  }
  ...
  if (hasOwner && (body.role ?? "owner") === "owner") {
    return NextResponse.json({ error: "Territory already has an active owner. Transfer first." }, { status: 409 });
  }
  ...
  await supabase.rpc("seed_eos_territory", { p_slug: body.TerritorySlug });

  // Trigger background market research (non-blocking)
  runTerritoryMarketResearch(body.TerritorySlug).catch((err) => {
