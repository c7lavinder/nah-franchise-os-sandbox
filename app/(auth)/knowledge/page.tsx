...
// Virtual pillar so the pillar view can render cross-cutting categories (mirrors native's ?pillar=cross)
const CROSS_PILLAR: Pillar = {
  key: "cross",
  label: "Cross-cutting",
  subtitle: "Brand, Operations, Planning, Governance",
  icon: Shield,
  gradient: "from-gray-500/10 to-gray-600/5",
  iconBg: "bg-gray-500",
  borderColor: "border-gray-200",
  categories: CROSS_CUTTING,
};

function findPillar(key: string | null): Pillar | undefined {
  if (key === "cross") return CROSS_PILLAR;
...
export default function KnowledgePage() {
  ...
  async function handleDelete(id: string) {
    ...
    if (activeDocId === id) {
      setActiveDocId(null);
      setView(activePillar ? "pillar" : "dashboard");
    }
    await fetchDocs();
  }

  // Navigation
  function openPillar(key: string) {
    setActivePillar(key);
    setView("pillar");
    setActiveDocId(null);
  }
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)]">
          ...
          <div>
            ...
            {view === "dashboard" && (
              <p className="text-[11px] text-text-tertiary">
                {activeDocs.length} documents powering Scout across 4 growth pillars
              </p>
            )}
          </div>
        </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                else if (view === "search") setView("dashboard");
              }}
      ...
      {(showAdd || editingId) && (
        <div className="mb-6 p-5 bg-white border border-border-default rounded-xl shadow-sm">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  min={1}
                  max={10}
                  className="input text-body-sm w-16"
                  value={formPriority}
            ...
            <textarea
              placeholder="Document content — this powers Scout's intelligence..."
              className="input text-body-sm w-full h-48 resize-y font-mono"
              value={formContent}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  onClick={cancelForm}
                  className="px-3 py-1.5 text-caption text-text-tertiary hover:text-text-primary rounded-lg transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="btn-primary text-caption px-4 py-1.5 flex items-center gap-1"
                >
            ...
            <div className="rounded-xl border border-border-default bg-white p-5">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[15px] font-bold text-scout-purple">
                    {activeDocs.filter((d) => d.seeded_from === "call_extraction").length}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-text-secondary">Human-Curated</span>
                  <span className="text-[15px] font-bold text-nah-blue">
                    {activeDocs.filter((d) => d.seeded_from !== "call_extraction").length}
                  </span>
                </div>
                ...
                .map((doc) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`px-1.5 py-0.5 rounded text-[9px] font-medium border ${categoryColor(doc.category)}`}
                      >
                        {categoryLabel(doc.category)}
                      </span>
                      <span className={`text-[10px] font-medium ${fresh.color} w-14 text-right`}>
      ...
      {view === "pillar" &&
        activePillar &&
        (() => {
          const pillar = findPillar(activePillar);
          ...
          return (
            <div className="space-y-4">
              {/* Pillar Header */}
              <div
                className={`rounded-xl border ${pillar.borderColor} bg-gradient-to-r ${pillar.gradient} p-5 flex items-center gap-4`}
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[12px] text-text-tertiary">
                    {pillar.subtitle} — {pDocs.length} documents
                  </p>
                </div>
                <button
                  onClick={() => startAdd(pillar.categories[0]?.value)}
                ...
                return (
                  <div key={cat.value} className="rounded-xl border border-border-default bg-white overflow-hidden">
                    <div className="px-4 py-3 border-b border-border-default bg-bg-secondary/50 flex items-center gap-2">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${categoryColor(cat.value)}`}
                      >
                        {cat.label}
                      </span>
                    ...
                    {catDocs.length === 0 ? (
                      <div className="px-4 py-6 text-center">
                        <p className="text-[12px] text-text-tertiary">
                          No documents yet. Scout will auto-populate from calls, or add one manually.
                        </p>
                      </div>
                    ) : (
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${categoryColor(activeDoc.category)}`}
                  >
              ...
              <div className="flex items-center gap-1.5 ml-4">
                <button
                  ...
                  className="px-2.5 py-1 text-[11px] font-medium text-nah-blue hover:bg-blue-50 rounded-lg transition-colors flex items-center gap-1"
                >
                  <Pencil size={12} /> Edit
                </button>
                <button
                  onClick={() => void handleDelete(activeDoc.id)}
                  className="px-2.5 py-1 text-[11px] font-medium text-red-500 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-1"
                >
          ...
          <div className="rounded-xl border border-border-default bg-white p-6">
            <div className="prose prose-sm max-w-none">
              {activeDoc.content.split("\n").map((line, i) => {
                if (line.startsWith("# "))
                  return (
                    <h1 key={i} className="text-[18px] font-bold text-text-primary mt-4 mb-2">
                      {line.slice(2)}
                    </h1>
                  );
                if (line.startsWith("## "))
                  return (
                    <h2 key={i} className="text-[15px] font-bold text-text-primary mt-4 mb-1.5">
                      {line.slice(3)}
                    </h2>
                  );
                if (line.startsWith("### "))
                  return (
                    <h3 key={i} className="text-[13px] font-semibold text-text-primary mt-3 mb-1">
                      {line.slice(4)}
                    </h3>
                  );
                if (line.startsWith("- **")) {
                  ...
                  if (match)
                    return (
                      <div key={i} className="flex gap-2 py-0.5 pl-3">
                        <span className="text-[12px] font-semibold text-text-primary">{match[1]}</span>
                        <span className="text-[12px] text-text-secondary">{match[2]}</span>
                      </div>
                    );
                }
                if (line.startsWith("- [ ] "))
                  return (
                    <div key={i} className="flex items-center gap-2 py-0.5 pl-3">
                      <div className="w-3.5 h-3.5 rounded border border-border-default" />
                      <span className="text-[12px] text-text-secondary">{line.slice(6)}</span>
                    </div>
                  );
                if (line.startsWith("- [x] ") || line.startsWith("- [X] "))
                  return (
                    <div key={i} className="flex items-center gap-2 py-0.5 pl-3">
                      ...
                      <span className="text-[12px] text-text-secondary line-through">{line.slice(6)}</span>
                    </div>
                  );
                if (line.startsWith("- "))
                  return (
                    <div key={i} className="flex gap-2 py-0.5 pl-3">
                      ...
                      <span className="text-[12px] text-text-secondary">{line.slice(2)}</span>
                    </div>
                  );
                if (line.startsWith("> "))
                  return (
                    <div key={i} className="border-l-2 border-scout-purple/30 pl-3 py-0.5 my-1">
                      <span className="text-[12px] italic text-text-tertiary">{line.slice(2)}</span>
                    </div>
                  );
                if (line.startsWith("_") && line.endsWith("_"))
                  return (
                    <p key={i} className="text-[11px] italic text-text-tertiary my-1">
                      {line.slice(1, -1)}
                    </p>
                  );
                ...
                if (line.startsWith("|"))
                  return (
                    <div
                      key={i}
                      className="text-[11px] text-text-secondary font-mono bg-bg-secondary/50 px-2 py-0.5 rounded"
                    >
                      {line}
                    </div>
                  );
