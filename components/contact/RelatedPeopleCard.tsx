        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {onAddRequested && (
            <button
              ...
              className="text-caption text-nah-blue hover:underline flex items-center gap-0.5"
            >
              <Plus size={11} /> Add
            </button>
          )}
        </div>
        {list.length === 0 && <p className="text-caption text-text-tertiary py-2">No contacts on this journey.</p>}
          ...
          {list.map((m) => {
            ...
            return (
              <div key={m.contact_id} className="flex items-center gap-2.5 py-1.5">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold flex-shrink-0 ${isCore ? "bg-nah-orange/15 text-nah-orange" : "bg-nah-blue/10 text-nah-blue"}`}
                >
                  {initials}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    {nameNode}
                    <span
                      className={`text-[9px] px-1 py-0.5 rounded ${isCore ? "bg-nah-orange/10 text-nah-orange" : "bg-text-tertiary/10 text-text-tertiary"}`}
                    >
                      {roleLabel}
                    </span>
                  </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          ...
          <h3 className="text-[10px] font-semibold text-text-tertiary tracking-wider">
            CONTACTS ({people.length + (mainContact ? 1 : 0)})
          </h3>
        </div>
        <button
      ...
      {showForm && (
        <div className="mb-3 p-3 bg-bg-secondary border border-border-default rounded-lg space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="btn-ghost px-3 py-1 text-caption">
              Cancel
            </button>
            <button
      ...
      {mainContact &&
        (() => {
          const mcName =
            capitalizeName(`${mainContact.first_name ?? ""} ${mainContact.last_name ?? ""}`.trim()) || "Unknown";
          const mcInitials = mcName
            .split(" ")
            .map((n) => n[0])
            .join("")
            .toUpperCase()
          ...
          return (
            <div className="flex items-center gap-2.5 py-1.5 mb-1 border-b border-border-default/50 pb-2">
              <div className="w-7 h-7 rounded-full bg-nah-orange/15 text-nah-orange flex items-center justify-center text-[10px] font-semibold flex-shrink-0">
                {mcInitials}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
          ...
          return (
            <div key={p.id} className="flex items-center gap-2.5 py-1.5 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-caption font-medium text-text-primary truncate">{name}</span>
                  <span className="text-[9px] px-1 py-0.5 rounded bg-text-tertiary/10 text-text-tertiary">
                    {ROLE_LABELS[p.role] ?? p.role}
                  </span>
                  {p.is_primary_decision_maker && (
                    <span className="text-[9px] px-1 py-0.5 rounded bg-nah-orange/10 text-nah-orange">Primary</span>
                  )}
                </div>
                <div className="flex items-center gap-2 text-[10px] text-text-tertiary">
      ...
      {deleteId && (
        <ConfirmModal
          ...
          body="Remove this related contact?"
          destructive
          confirmLabel="Remove"
