...
export default function CallTypesRubricEditor() {
  ...
  async function handleAddCallType() {
    const name = prompt("New call type name:");
    if (!name?.trim()) return;
    if (await apiCall("/api/settings/call-types", "POST", { name: name.trim() })) {
      await fetchCallTypes();
    }
  }

  async function handleDeleteCallType(id: string) {
  ...
  async function handleAddCriterion() {
    ...
    const name = prompt("New criterion name (e.g., Rapport, Discovery, Objection Handling):");
    if (!name?.trim()) return;
    // Get rubric ID
    ...
    if (await apiCall(`/api/settings/rubrics/${data.rubric.id}/criteria`, "POST", { name: name.trim() })) {
