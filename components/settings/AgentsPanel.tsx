...
const FALLBACK_CATEGORIES: AgentCategory[] = [
  ...
  { key: "marketing", label: "Lead Generation", summary: "Inbound, outbound, and content support." },
...
const agentPortraitVariants: Record<string, number> = {
  ...
  "workflow-qa": 9,
  "eos-sync": 10,
  "campaign-composer": 11,
  "outbound-lead-generator": 12,
};
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-md border border-white/70 shadow-inner ${
        compact ? "h-20 w-16" : "aspect-[3/2] w-full"
      }`}
      style={{
    ...
    >
      <AgentCharacterPortrait
        ...
        className={
          compact
            ? "absolute bottom-0 left-1/2 h-20 w-16 -translate-x-1/2 rounded-none border-0 bg-transparent shadow-none"
...
export default function AgentsPanel() {
  ...
  const guardedCount = agents.filter(
    (agent) => agent.trustLevel === "approval-required" || agent.trustLevel === "manual-only"
  ).length;
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {categoryAgents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="rounded-md border border-border-default bg-bg-primary p-2.5"
                          style={{
                        ...
                        >
                          <div className="mb-2 flex items-center justify-between gap-2">
                            ...
                            <span
                              className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${statusStyle[agent.status]}`}
                            >
                          ...
                          <div className="mt-2 rounded-md bg-white/90 p-2.5 shadow-sm">
                            <button
                              type="button"
                              onClick={() => setActiveAgentName(agent.name)}
                              className="block min-w-0 text-left"
                            >
                              <h4 className="font-headline text-[22px] leading-tight text-text-primary">
                                {agent.label}
                              </h4>
                            ...
                            <p className="mt-2 min-h-[34px] text-body-sm text-text-secondary">{agent.mission}</p>
                          </div>

                          <div className="mt-2 grid grid-cols-3 overflow-hidden rounded-md border border-border-default bg-white text-center">
                            <div className="border-r border-border-default px-2 py-1.5">
                              <div className="text-[10px] font-bold uppercase tracking-wide text-text-tertiary">
                                Runs
                              </div>
                              <div className="text-body-sm font-black text-text-primary">
                                {metricLabel(agent.runsMTD)}
                              </div>
                            </div>
                            <div className="border-r border-border-default px-2 py-1.5">
                              <div className="text-[10px] font-bold uppercase tracking-wide text-text-tertiary">
                              ...
                              <div className="text-body-sm font-black text-text-primary">
                                {metricLabel(agent.suggestionsMTD)}
                              </div>
                            </div>
                            <div className="px-2 py-1.5">
                              ...
                              <div className="truncate text-[11px] font-black text-text-primary">
                                {trustLabel[agent.trustLevel]}
                              </div>
                            </div>
                          </div>

                          <div className="mt-2 space-y-1.5 rounded-md border border-border-default bg-white/75 p-2.5 text-caption text-text-secondary">
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="rounded-lg border border-border-default bg-bg-secondary p-3">
                  ...
                  <p className="mt-2 text-caption text-text-secondary">
                    Add corrections, rules, examples, tone preferences, and things this agent should remember before it
                    answers.
                  </p>
                  <textarea
                    ...
                    maxLength={8000}
                    onChange={(event) =>
                      activeAgent && setTrainingDrafts((prev) => ({ ...prev, [activeAgent.name]: event.target.value }))
                    }
                  ...
                  <div className="mt-2 flex items-center justify-between gap-3">
                    <div className="text-[11px] text-text-tertiary">
                      {trainingSavedAt
                        ? `Saved ${formatDate(trainingSavedAt)}`
                        : "Saved notes are injected into this agent's chat context."}
                    </div>
                    <button
                      type="button"
