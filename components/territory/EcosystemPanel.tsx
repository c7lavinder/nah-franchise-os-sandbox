...
export default function EcosystemPanel({ msSlug, owner }: Props) {
  ...
  useEffect(() => { void fetchStakeholders(); }, [fetchStakeholders]);
      ...
      <div className="relative bg-bg-primary border border-border-default rounded-xl p-6">
        <div className="flex flex-col items-center">
          ...
          {stakeholders.length === 0 && !loading && (
            <p className="text-caption text-text-tertiary py-4">No ecosystem stakeholders yet. Add agents, contractors, partners, and more.</p>
          )}
          ...
          {stakeholders.map((s) => {
            ...
            return (
              <div key={s.id} className="grid grid-cols-[1fr_100px_120px_120px_32px] gap-2 px-4 py-2.5 border-t border-border-default items-center hover:bg-bg-hover/30 transition-colors">
                ...
                <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium w-fit border ${roleDef?.color ?? "bg-gray-100 text-gray-600 border-gray-200"}`}>
                ...
                <button onClick={() => void handleRemove(s.id)} className="p-1 text-text-tertiary hover:text-danger transition-colors">
                  <Trash2 size={12} />
                </button>
              </div>
            );
      ...
      {showForm ? (
        <div className="border border-border-default rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowForm(false)} className="text-text-tertiary hover:text-text-primary"><X size={14} /></button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <input value={form.first_name} onChange={(e) => setForm({ ...form, first_name: e.target.value })} placeholder="First name" className="bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
            <input value={form.last_name} onChange={(e) => setForm({ ...form, last_name: e.target.value })} placeholder="Last name" className="bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
            <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
            <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Phone" className="bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
            <input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} placeholder="Company" className="bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
          ...
          <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Notes (optional)" className="w-full bg-bg-secondary border border-border-default rounded-lg px-3 py-2 text-body-sm" />
          <button onClick={() => void handleAdd()} disabled={saving || (!form.first_name.trim() && !form.last_name.trim())}
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 text-caption text-nah-blue hover:text-blue-700 transition-colors">
